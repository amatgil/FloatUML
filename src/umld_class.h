...
void umld_enum(struct Enumeration es, struct Style *style) {
    Vector2 m_enum = MeasureTextEx(style->font, "<<enumeration>>",
        style->fontsize, 0);
    ...
    Vector2 size_rect = {
        m_enum.x,
        (ntipos + 1) * max + marge};
    Vector2 m_nom = MeasureTextEx(style->font, es.nom,
                style->fontsize, 0);
    if (m_nom.x > max) max = m_nom.x;

    for (uint32_t i = 0; i < ntipos; ++i) {
        Vector2 m_typ = MeasureTextEx(
            style->font, es.tipus[i].text, style->fontsize, 0);
    ...
    DrawTextEx(style->font, es.nom, (Vector2){es.pos.x + marge/2, es.pos.y + marge/2}, style->fontsize, 0, BLACK);
    for (uint32_t i = 0; i < ntipos; ++i) {
        Vector2 m_typ = MeasureTextEx(
            style->font, es.tipus[i].text, style->fontsize, 0);
        ...
        DrawTextEx(style->font, es.tipus[i].text, pos,
                   style->fontsize, 0, BLACK);
        pos.x += m_enum.x;
    }
    ...
    p_i = (Vector2){es.pos.x, es.pos.y + (ntipos + 1) * m_enum.y + m_enum.y/3};
    DrawLineEx(p_i, p_f, 3.5, BLACK); // horitzontal 3
